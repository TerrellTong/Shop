...
<html>
	<head>
		...
		<script>
			function delPro(pid) {
				if(confirm("您是否要删除这个商品？")){
					window.location.href = "${pageContext.request.contextPath}/deleProByCart?pid="+pid;
				}
			}

			function clearPro() {
				if(confirm("您是否要清空所有购物车？")){
					window.location.href = "${pageContext.request.contextPath}/clearProByCart";
				}
			}
		</script>
	</head>
	...
	<body>
		...
		<div class="container">
						...
						<tbody>
							...
							<c:forEach items="${cart.map}" var="entry">
								<tr class="active">
									...
									<td width="10%">
											${entry.value.buyNumber}
									</td>
									<td width="15%">
										<span class="subtotal">￥${entry.value.currentMoney}</span>
									</td>
									<td>
										<a href="javascript:void(0);" onclick="delPro('${entry.value.product.pid}')" class="delete">删除</a>
									</td>
								</tr>
							</c:forEach>
						</tbody>
			...
			<div style="margin-right:130px;">
				...
				<div style="text-align:right;margin-top:10px;margin-bottom:10px;">
					<a href="javascript:void(0);" onclick="clearPro()" id="clear" class="clear">清空购物车</a>
		...
		<c:if test="${ empty cart.map}">
			您的购物车是空的</br>
			<a href="${pageContext.request.contextPath}/indexServlet">返回首页</a>
		</c:if>
		<!-- 引入footer.jsp -->
